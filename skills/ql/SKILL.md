---
name: ql
description: Log questions for leadership. Deduplicates, categorizes, prioritizes, and optionally consults /exec-review for strategic questions.
user-invocable: true
allowed-tools: Read, Write, Edit, Grep, Glob, Skill, Agent
argument-hint: "<question or topic>"
reads:
  - docs/strats/questions4leadership.md
---

# /ql — Questions for Leadership

Log, categorize, and manage questions for leadership. Questions accumulate between meetings and are reviewed at regular leadership meetings.

## Trigger

User invokes `/ql <question>` with a question, topic, or concern.

Special modes:
- `/ql` (no args) — Show current batch summary
- `/ql review` — Review all pending questions, suggest priority order for next meeting
- `/ql answered <QL-ID> <notes>` — Mark a question as answered with brief notes
- `/ql defer <QL-ID>` — Move to deferred
- `/ql archive` — Move all ANSWERED/DEFERRED to archive section

## Process

### Step 1 — Read Current State
Read `docs/strats/questions4leadership.md` to understand:
...
- **TECH** — Technical (infrastructure, legacy platform, architecture, tooling)
...
- Challenging an assumption leadership have made
...
- Ask what the plan owner already knows vs. what leadership uniquely can answer

Keep it brief — 1-2 clarifying questions max. Don't interrogate.

### Step 7 — Log the Question
Add to the **Current Batch** section of `docs/strats/questions4leadership.md` in this format:
...
**Related:** [QL-NNN, QL-NNN — linked questions on the same topic]
**Owner:** [Who follows up — defaults to plan owner unless someone else is better positioned]
**Question:** [The question, clearly framed for leadership]
...
Number sequentially (QL-001, QL-002, etc.). Check the file for the last used number.

### Step 8 — Confirm
Show the user the logged question and ask if the framing is right. Leadership is busy — questions should be:
...
6. Output a clean agenda format leadership can scan in 30 seconds

## Answer Mode (`/ql answered <ID> <notes>`)
...
- **Frame for leadership, not for Claude.** Leadership is busy transitioning out. Questions should be clear, actionable, and respect his time.
