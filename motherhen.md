# Project Health & Compliance Monitor

Usage: `/motherhen [mode]`

**Modes:**
- *(none)* or `full` — Run all applicable checks
- `quick` — Run only universal checks (context, release, foundation)
- `focus:<check>` — Run a single check in depth (e.g., `focus:tests`, `focus:security`, `focus:sprint`)
...
| Type | Collection Command | Count Extraction |
|------|-------------------|-----------------|
| Python (pytest) | `pytest tests/ --co -q 2>/dev/null \| tail -1` | Parse "X tests collected" |
| Node (jest) | `npx jest --listTests 2>/dev/null \| wc -l` | Count test files |
| Node (vitest) | `npx vitest --run --reporter=json 2>/dev/null` | Parse results |
| Go | `go test ./... -list '.*' 2>/dev/null \| grep -c '^Test'` | Count test functions |
| Rust | `cargo test -- --list 2>/dev/null \| grep -c ': test'` | Count tests |
| .NET | `dotnet test --list-tests 2>/dev/null \| grep -c '^ '` | Count tests |
...
| 6 | PM Framework | `docs/pm/FRAMEWORK.md`, `docs/PM-FRAMEWORK.md` |
| 7 | Workflow Guide | `docs/standards/SOLO-AI-WORKFLOW-GUIDE.md`, `docs/SOLO-AI-WORKFLOW-GUIDE.md` |
| 8 | CI/CD Proposal | `docs/standards/CICD-DEPLOYMENT-PROPOSAL.md`, `docs/CICD-DEPLOYMENT-PROPOSAL.md` |
| 9 | Multi-Developer Guide | `docs/standards/MULTI-DEVELOPER-GUIDE.md`, `docs/MULTI-DEVELOPER-GUIDE.md` |
| 10 | Infrastructure Playbook | `docs/standards/INFRASTRUCTURE-PLAYBOOK.md`, `docs/INFRASTRUCTURE-PLAYBOOK.md` |
| 11 | Cost Management Guide | `docs/standards/COST-MANAGEMENT-GUIDE.md`, `docs/COST-MANAGEMENT-GUIDE.md` |
| 12 | Security Review Protocol | `docs/standards/SECURITY-REVIEW-PROTOCOL.md`, `docs/SECURITY-REVIEW-PROTOCOL.md` |
| 13 | Ops Readiness Checklist | `docs/standards/OPS-READINESS-CHECKLIST.md`, `docs/OPS-READINESS-CHECKLIST.md` |
| 14 | AI-DLC Case Study | `docs/AI-DLC-CASE-STUDY.md`, `docs/standards/AI-DLC-CASE-STUDY.md` |
